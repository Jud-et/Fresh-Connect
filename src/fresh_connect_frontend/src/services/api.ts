...
import { _SERVICE } from '../../../declarations/fresh_connect_backend/fresh_connect_backend.did';
...
export const api = {
  registerUser: (username: string, role: 'farmer' | 'consumer' | 'mamamboga') => 
    backendActor.registerUser(username, { [role]: null }),

  getUser: (userId: string) => backendActor.getUser(userId),
  ...
  searchProducts: (productType?: string, maxPrice?: bigint) =>
    backendActor.searchProducts([productType], [maxPrice]),
  ...
  updateOrderStatus: (orderId: bigint, newStatus: Order['status']) =>
    backendActor.updateOrderStatus(orderId, { [newStatus]: null }),
  ...
  getUserAnalytics: (userId: string) => backendActor.getUserAnalytics(userId),

  getFarmerAnalytics: (farmerId: string) => backendActor.getFarmerAnalytics(farmerId),
};